 ...
 * @author Vinxce (Little edits by Nelderson)
...
(function() {
	...
	MMO_Scene_Title.prototype.start = function() {
	    ...
	    this.createLoginForm();
			//this.createRegistrationForm();//NEL TEST
	};
		...
		$("#ErrorPrinter").html(
			'<div id="RegisterForm" class="panel panel-primary" style="width:'+(Graphics.boxWidth - (Graphics.boxWidth / 3))+'px">'+
				...
				'<div class="panel-body">'+
					...
					//'<form role="form" action="http://localhost:3000/register" method="post" style="max-width:300px;margin:auto;text-align:center;">'+
	...
	MMO_Scene_Title.prototype.createActivationForm = function() {
		$("#ErrorPrinter").html(
			'<div id="ActivationForm" class="panel panel-primary" style="width:'+(Graphics.boxWidth - (Graphics.boxWidth / 3))+'px">'+
				...
				'<div class="panel-body">'+
					...
					'<div class="input-group">'+
						'<span class="input-group-addon" id="username-addon"><i class="fa fa-user"></i></span>'+
						'<input type="text" class="form-control login-input" id="actcode" placeholder="Activation Code" aria-describedby="username-addon">'+
					'</div><br>'+
					'<button id="btnActSubmit" class="btn btn-primary">Submit</button>'+
				'</div>'+
			'</div>');
		...
		$("#btnActSubmit").click(function(){that.activationAttempt();});
		//$("#btnRegister").click(function(){that.createRegistrationForm();});
	};
	...
	MMO_Scene_Title.prototype.connectAttempt = function(){
		...
		$socket.on("loginResponse", function(data){
			...
			if (data.msg && data.msg == username) {
				...
				console.log($gameSystem._username);
				that.fadeOutAll();
			    SceneManager.goto(Scene_Map);
				return that.displayInfo("Ok : "+data.msg);
			}
		});
		$socket.emit("loginQuery", {
			username: username,
			password: shapwd
		});
	};
	...
	MMO_Scene_Title.prototype.activationAttempt = function(){
		...
		if (actcode.length === 0) return this.displayError("You must provide a activation code!");
		...
		$socket.on("activationResponse", function(data){
			...
			if (data.msg==='success') {
				...
				// return that.displayInfo("Check your email for the activation code!");
			}
		});
		...
		$socket.on("registerResponse", function(data){
			...
			if (data.msg && data.msg == username) {
			    ...
			    //SceneManager.goto(Scene_Map);
					$gameSystem._username = username;
					that.createActivationForm();
				that.displayInfo("Check your email for the activation code!");
			}
		});

		$socket.emit("registerUser", {
			username: username,
			password: password, //NEL test
			email: email
		});
	};
